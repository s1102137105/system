<?php
    ...
    if($_SESSION['id']==null)
    {
        header("Location:login.html");
            ...
            if($_POST['search']==null && $_POST['searchtext']==null)
    {
    ...
    try{
        ...
        $sql="SELECT * FROM `db_receipt` a join `db_student`b on a.stu_ID=b.Student_ID join `db_employee` c on a.Emp_ID =c.Employee_ID  WHERE `$search` like '%$searchtext%'";
        $stmt=$conn->query($sql);
