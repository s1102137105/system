<?php
    ...
    try{
        ...
        $sql="SELECT * FROM `db_receipt` a join `db_student`b on a.stu_ID=b.Student_ID join `db_employee` c on a.Emp_ID =c.Employee_ID WHERE a.`Receipt_ID`='$Receipt_ID'";
        $stmt=$conn->query($sql);
...
?>
        ...
        <form action="rpt_updatepage1.php" method="post" name="a" onsubmit="return check()">
        <input type="hidden" name="Receipt_ID" value="<?php echo $row['Receipt_ID'];?>"/>
        <table>
        <tbody>
            <tr>
                <td>項目</td>
                <td><input type="text" name="Receipt_Project" value="<?php echo $row['Receipt_Project'];?>"/></td>
            </tr> 
            <tr>
                <td>金額</td>
                <td><input type="text" name="Receipt_Money" value="<?php echo $row['Receipt_Money'];?>"/></td>
            </tr> 
            <tr>
                <td>日期</td>
                <td><input type="date" name="Receipt_Date" value="<?php echo $row['Receipt_Date'];?>"/></td>
            </tr>
            <tr>
                <td>收款人</td>
                <td>
