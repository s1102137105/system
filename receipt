<?php
    ...
    try{
        ...
        $sql="SELECT * FROM `db_receipt` a join `db_student`b on a.stu_ID=b.Student_ID join `db_employee` c on a.Emp_ID =c.Employee_ID order by `Receipt_Date`";
        $stmt=$conn->query($sql);
...
<article id="post-47" class="post-47 page type-page status-publish hentry">
	...
	<header class="entry-header">
		<h1 class="entry-title">收據</h1>	</header><!-- .entry-header -->
                <?php
        if($_SESSION['position']!="總務"  &&  $_SESSION['position']!="管理者" && $_SESSION['position']!="會計" && $_SESSION['position']!="櫃台人員")
        {
            echo "你的權限不足無法瀏覽";
        }
        else{
?>
<div align="right">
        <form action="rpt_search.php" method="POST">
            <select name="search">
                <option value="Receipt_Project">項目</option>
                <option value="Receipt_Money">金額</option>
                <option value="Receipt_Date">日期</option>
                <option value="Employee_Name">收款人</option>
                <option value="Student_Name">繳款人</option>
            </select>
    ...
    <tr>
        <td><?php echo $row['Receipt_Project'];?></td>
        <td><?php echo $row['Receipt_Money'];?></td>
